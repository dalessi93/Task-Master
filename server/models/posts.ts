...
export const Posts = {
    ...
    //         });
    // },
    getByFilter: (category: any, suburb: any, state: any, id: any) => {
        let query = `SELECT * FROM posts INNER JOIN users on posts.user_id = users.user_id`;
        let params = [];
        let where = [];
        ...
        if(id){
            params.push(id);
            where.push(`posts.user_id = $${params.length}`);
        }
